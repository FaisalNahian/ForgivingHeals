(function(Views, Models, $, Backbone) {
			...
			this.uploader = new AE.Views.File_Uploader({
				...
				cbUploaded: function(up, file, res) {
					if (res.success) {
						...
						view.closeModal();
						$("span.filename").text(forgivingheals_front.texts.no_file_choose);
						up.splice();
						up.refresh();
						up.destroy();
					} else {
	...
	Views.AuthModal = AE.Views.Modal_Box.extend({
		...
		resetAuthForm: function(event) {
			...
			$('.modal-title-sign-in').empty().text(forgivingheals_front.texts.sign_in);
		...
		doLogin: function(event) {
			...
			this.login_validator = $("form#signin_form").validate({
				...
				messages: {
					username: forgivingheals_front.form_auth.error_msg,
					password: forgivingheals_front.form_auth.error_msg,
				}
			});
		...
		doRegister: function(event) {
			...
			this.register_validator = $("form#signup_form").validate({
				...
				messages: {
					username: {
						required: forgivingheals_front.form_auth.error_msg,
						username: forgivingheals_front.form_auth.error_username,
					},
					password: forgivingheals_front.form_auth.error_msg,
					email: {
						required: forgivingheals_front.form_auth.error_msg,
						email: forgivingheals_front.form_auth.error_email,
					},
					re_password: {
						required: forgivingheals_front.form_auth.error_msg,
						equalTo: forgivingheals_front.form_auth.error_repass,
					}
				}
			});
	...
	Views.ResetPassModal = AE.Views.Modal_Box.extend({
		...
		doResetPassword: function(event) {
			...
			this.reset_validator = $("form#resetpass_form").validate({
				...
				messages: {
					new_password: forgivingheals_front.form_auth.error_msg,
					re_new_password: {
						required: forgivingheals_front.form_auth.error_msg,
						equalTo: forgivingheals_front.form_auth.error_repass,
					}
	...
	Views.EditStoryModal = AE.Views.Modal_Box.extend({
		events: {
			// user submit story info
			'submit form#submit_story'	: 'saveStory',
			// user hit enter to add new tag
			'keypress input#story_tags'	: 'onAddTag',
		},
		// initialize modal story
		...
		initialize: function() {
			...
			this.blockUi = new AE.Views.BlockUi();
			/**
			 * init editor control to story content
			 */
			tinymce.EditorManager.execCommand("mceAddEditor", false, "insert_story");
			...
			$('#story_tags').typeahead({
				...
				source: function(query, process) {
					...
					return $.getJSON(
						ae_globals.ajaxURL, {
							action: 'forgivingheals_get_tags'
						},
						function(data) {
							//console.log(data);
							view.tags = data;
		...
		initContent: function(model) {
			...
			var $forminfo = this.$("form#submit_story"),
				// story tags
				tags     = this.model.get("forgivingheals_tag"),
				// story category
				category = this.model.get('story_category'),
				view     = this;
			...
			setTimeout(function() {
				...
				if (typeof category !== 'undefined' && category.length > 0) {
					view.$('#story_category').val(category[0].slug);
				}
				/**
				 * setup edit content
				 */
				if (model.get('content_edit')) {
					tinymce.get('insert_story').setContent(model.get('content_edit'));
				} else {
					tinymce.get('insert_story').setContent('');
				}
		...
		validate: function() {
			 ...
			 */
			this.submit_validator = $("form#submit_story").validate({
				ignore: "",
				rules: {
					post_title: "required",
				...
				messages: {
					post_title: forgivingheals_front.form_auth.error_msg,
					story_category: forgivingheals_front.form_auth.error_msg,
					post_content: forgivingheals_front.form_auth.error_msg,
				}
			});
		},
		...
		saveStory: function(event) {
			...
			var form = $(event.currentTarget),
				$button = form.find("button#btn_submit_story"),
				data = form.serializeObject(),
				view = this;

			// if (this.tag_list.find('li').length == 0) { // user should enter at least on tag
			// 	$("input#story_tags").attr('placeholder', 'Please insert at least one tag.').css('border', '1px solid red');
			// }

			if (this.submit_validator.form() && tinymce.get('insert_story').getContent() != "" /*&& this.tag_list.find('li').length > 0*/ ) {

				this.model.set('content', data);
				this.model.save('do_action', 'saveStory', {
					beforeSend: function() {
						view.blockUi.block($button);
						//console.log('chay');
					},
		...
		addTag: function(tag) {
			...
			if (duplicates.length == 0 && tag != '' && count.length < 5) {
				...
				$('input#story_tags').val('').css('border', '1px solid #dadfea');;
			}
		},
		...
		onAddTag: function(event) {
			...
			if (event.which == 13) {
				...
				if (typeof caps['create_tag'] === 'undefined' && $.inArray(val, this.tags) == -1) {
					this.$('#story_tags').popover({
						content: this.$('#add_tag_text').val(),
						container: '#modal_submit_stories'
					});
					return false;
				}
				/**
		...
		submitReport: function(event){
			...
			this.model.save('data',data, {
				beforeSend: function() {
					if( message == '' ){
						AE.pubsub.trigger('ae:notification', {
							msg: forgivingheals_front.form_auth.error_msg,
							notice_type: 'error',
						});
						return false;
					}
	...
	Views.ContactModal = AE.Views.Modal_Box.extend({
		...
		validate: function(){
			this.submit_validator = $("form#contact_form").validate({
				...
				messages: {
					txt_msg: forgivingheals_front.form_auth.error_msg
				}
			});
		},
	...
	Views.PostListItem = Backbone.View.extend({
		tagName: 'div',
		className: 'row story-main-content story-item reaction-item',
		model: [],
		events: {
		...
		initialize: function() {
			if ($('#reaction_item').length > 0) {
				this.template = _.template($('#reaction_item').html());
			}
			this.blockUi = new AE.Views.BlockUi();
		},
		...
		showListComment: function(event) {
			...
			var target = $(event.currentTarget),
				...
				editorId   = "insert_reaction_" + this.model.get('id'),
			...
			if (countCmt == 0) {
				...
				$('a.add-comment').addClass('clicked').text(forgivingheals_front.texts.add_comment).show().removeClass('clicked');
				this.$('a.add-comment').hide().addClass('clicked').text(forgivingheals_front.texts.cancel);
				$("div.child-reaction-wrap").slideUp('fast');
		...
		doAction: function(event) {
			...
			if (currentUser.ID == 0) {
				AE.pubsub.trigger('ae:notification', {
					msg: forgivingheals_front.texts.require_login,
					notice_type: 'error',
				});
				return false;
			}
			...
			if (currentUser.is_ban) {
				AE.pubsub.trigger('ae:notification', {
					msg: forgivingheals_front.texts.banned_account,
					notice_type: 'error',
				});
				return false;
			}
			...
			if (action != 'edit' && typeof userCaps[action] === 'undefined' &&
				...
				!(action == 'unfollow' || action == 'follow' || action == 'delete' || action == 'accept-reaction' || action == 'un-accept-reaction' || action == "cancel-post-edit" || action == "report" || action == "approve") ) return false;
			...
			if (action == "vote_up" || action == "vote_down") {
				...
				/* ========== ON MARK ACCEPT ========== */
			} else if (action == "accept-reaction" || action == "un-accept-reaction") {
				if (action == "un-accept-reaction" && currentUser.ID != currentStory.post_author)
					return false;
				if (target.hasClass('active')) {
					target.removeClass('active')
				} else {
					$('a.accept-reaction').removeClass('active');
					target.addClass('active');
				}
				/* ========== ON EDIT POST ========== */
			} else if (action == "edit") {
				if (this.model.get('post_type') == 'story') {

					if ( typeof userCaps['edit_story'] === 'undefined' && currentUser.ID != this.model.get("post_author") )
						return false;

					if (typeof this.Modal_Edit === 'undefined') {
						this.Modal_Edit = new Views.EditStoryModal({
							model: this.model,
							el: $('#modal_submit_stories')
						});
					}
				...
				if ( typeof userCaps['edit_reaction'] === 'undefined' && currentUser.ID != this.model.get("post_author") ) return false;
				...
				view.$el.find('div.story-content').fadeOut('fast', function() {
				...
				view.$el.find('div.post-content-edit').fadeOut('fast', function() {
					view.$el.find('div.story-content').fadeIn();
				...
				this.model.destroy({
					...
					success: function(result, status, jqXHR) {
						...
						if (status.success) {
							...
							//update html
							var count = parseInt($('.reactions-count .number').text());
								count--;
							$('.reactions-count .number').text(count >= 1 ? count : 0);


							if (result.get('post_type') == "story")
								window.location.href = status.redirect;
							else
								view.$el.fadeOut();
						} else {
				...
				if (this.model.get('post_type') == 'story' || this.model.get('post_type') == 'reaction') {
					this.onReport(event);
					return false;
				}

				if (typeof userCaps['edit_reaction'] === 'undefined') return false;
			...
			//approve pending reaction
			else if (action == "approve") {
				//check if reaction only
		...
		onSubmitEdit: function(event) {
			...
			this.model.save('', '', {
				...
				success: function(result, status, jqXHR) {
					...
					if (status.success) {
						view.$el.find('div.post-content-edit').fadeOut('fast', function() {
							view.$el.find('div.story-content').html(result.get('content_filter'));
		...
		hideCommentForm: function(event) {
			...
			this.$('a.add-comment').text(forgivingheals_front.texts.add_comment).removeClass('clicked');
		...
		showCommentForm: function(event) {
			...
			var target = $(event.currentTarget),
				...
				editorId = "insert_reaction_" + id,
				wrapId = "#editor_wrap_" + id;

			if (currentUser.ID == 0) {
				//bootbox.alert(forgivingheals_front.texts.require_login);
				AE.pubsub.trigger('ae:notification', {
					msg: forgivingheals_front.texts.require_login,
					notice_type: 'error',
				});
			...
			$('a.add-comment').text(forgivingheals_front.texts.add_comment).show();
			$("div.child-reaction-wrap").slideUp('fast');

			if (target.hasClass('clicked')) {
				target.removeClass('clicked');
				$('a.add-comment').text(forgivingheals_front.texts.add_comment);
				$(wrapId).slideUp('fast', function() {
					if (typeof tinymce.activeEditor != "undefined")
				...
				//create new tinymce
				target.text(forgivingheals_front.texts.cancel);
				tinymce.EditorManager.execCommand("mceAddEditor", false, editorId);
				tinymce.activeEditor.execCommand('mceSetContent', false, '');
				$(wrapId).slideDown();
			}
		...
		insertComment: function(event) {
			...
			if(ae_globals.user_confirm && currentUser.register_status == "unconfirm"){
				//bootbox.alert(forgivingheals_front.texts.confirm_account);
				AE.pubsub.trigger('ae:notification', {
					msg: forgivingheals_front.texts.confirm_account,
					notice_type: 'error',
				});
			...
			reaction.save('', '', {
				...
				success: function(result, status, jqXHR) {
					...
					if (status.success) {
						...
						view.$('.child-reaction-wrap').slideUp();
						view.$('a.add-comment').text(forgivingheals_front.texts.add_comment).removeClass('clicked').show();
		...
		onReport: function(event) {
			var view = this
			var Modal_Report = ForgivingHeals.App.getReportModal(this.model);
	...
	Views.EditProfileModal = AE.Views.Modal_Box.extend({
		...
		saveProfile: function(event) {
			...
			this.submit_validator = $("form#submit_edit_profile").validate({
				...
				messages: {
					display_name: forgivingheals_front.form_auth.error_msg,
					// user_location: forgivingheals_front.form_auth.error_msg,
					user_email: {
						required: forgivingheals_front.form_auth.error_msg,
						email: forgivingheals_front.form_auth.error_email,
					}
		...
		changePassword: function(event) {
			...
			this.change_pass_validator = this.$("form#submit_edit_password").validate({
				...
				messages: {
					old_password: forgivingheals_front.form_auth.error_msg,
					new_password: forgivingheals_front.form_auth.error_msg,
					re_password: {
						required: forgivingheals_front.form_auth.error_msg,
						equalTo: forgivingheals_front.form_auth.error_repass,
	...
	Views.Front = Backbone.View.extend({
		...
		currentUser: [],
		events: {
			'click button.share-story'			: 'openSubmitModal',
		...
		initialize: function() {
			...
			//validate tinymce
			$("#insert_story").on('change',function(event) {
				$(this).valid();
			});
		...
		openAgain: function(){
			...
			$('#modal_submit_stories').css('display','block');
		},
		openSubmitModal: function(event) {
			...
			if (typeof this.submitModal === 'undefined') {
				this.submitModal = new Views.EditStoryModal({
