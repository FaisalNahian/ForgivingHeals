jQuery(document).ready(function($) {
	...
	if( $("#reactions_filter").length > 0 ){
		$('#reactions_filter').waypoint('sticky', {
	...
	if( $("#story_filter").length > 0 ){
		$('#story_filter').waypoint(function(direction) {
			//console.log('aaa');
			if(direction == "down")
	...
	$(window).bind('beforeunload', function() {
		...
		if (hasChange) {
			return forgivingheals_front.texts.close_tab;
		}
	});
	...
	$(".link_sign_up").click(function(event) {
		...
		$('#signin_form').fadeOut("slow", function() {
			...
			$('.modal-title-sign-in').empty().text(forgivingheals_front.texts.sign_up);
	...
	$(".link_forgot_pass").click(function(event) {
		...
		$('#signin_form').fadeOut("slow", function() {
			...
			$('.modal-title-sign-in').empty().text(forgivingheals_front.texts.forgotpass);
