		...
		if ( FB ){
			FB.login(function(response) {
				if (response.authResponse) {
						...
						var params = {
							...
							success: function(resp){
								if ( resp.success && typeof resp.data.redirect_url != 'undefined' ){
									...
									// assign current user
									var model = new ForgivingHeals.Models.User(resp.data.user);
									ForgivingHeals.App.currentUser = model;

									// trigger events
									var view 	= ForgivingHeals.App.authModal;
					...
					AE.pubsub.trigger('ae:notification', {
						msg: forgivingheals_front.texts.cancel_auth,
						notice_type: 'error',
					});
				}
			}, {
