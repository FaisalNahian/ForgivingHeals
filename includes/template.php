...
/**
*
* HOOK TO TOP & BOTTOM STORY LISTING
* @param
* @author ThaiNT
* @since 1.0
*
**/
add_action( 'forgivingheals_top_quetions_listing', 'forgivingheals_top_quetions_listing' );
function forgivingheals_top_quetions_listing(){
	if(is_active_sidebar( 'forgivingheals-top-stories-banner-sidebar' )){
	?>
    <div class="row">
        <div class="col-md-12 ads-wrapper">
            <?php dynamic_sidebar( 'forgivingheals-top-stories-banner-sidebar' ); ?>
        </div>
...
add_action( 'forgivingheals_btm_stories_listing', 'forgivingheals_btm_stories_listing' );
function forgivingheals_btm_stories_listing(){
	if(is_active_sidebar( 'forgivingheals-btm-stories-banner-sidebar' )){
	?>
    <div class="row">
        <div class="col-md-12 ads-wrapper btm-ads-wrapper">
            <?php dynamic_sidebar( 'forgivingheals-btm-stories-banner-sidebar' ); ?>
        </div>
    </div><!-- END WIDGET BANNER -->
	<?php
	}
...
*
**/
function forgivingheals_template_share($id){
...
*
**/
function forgivingheals_template_paginations($query,$paged){
	global $wp_rewrite;
...
*
**/
function forgivingheals_option_categories_redirect($current = false, $args = array()){
	$current = get_query_var( 'term' );
	$args = wp_parse_args( $args, array(
		'hide_empty' => 0,
		'orderby'    => 'term_group'
	));
		$terms = get_terms( 'story_category', $args );
		foreach ($terms as $term) {
			$space = $term->parent ? '&nbsp;&nbsp;&nbsp;&nbsp;' : '';
	?>
	<option <?php echo $current == $term->slug ? 'selected' : ''; ?> value="<?php echo get_term_link($term, 'story_category' ); ?>">
		<?php
...
/**
*
* TEMPLATE FILTER STORIES LIST
* @param
* @author ThaiNT
* @since 1.0
*
**/
function forgivingheals_template_filter_stories(){
	...
	elseif(isset($_GET['sort']) && $_GET['sort'] == 'unreacted')
		$args = array('sort' => 'unreacted');
	else
		$args = array();

	$posts_per_page = apply_filters( 'forgivingheals_filter_numbers_array', array(20,15,10,5) );
...
?>
				...
				<div class="row">
					<div class="col-md-2 hidden-xs hidden-sm">
						...
						<span class="q-f-title">
							<?php _e("Search Stories", ET_DOMAIN) ?>
						</span>
						<?php } else { ?>
						<span class="q-f-title">
							<?php _e("All Stories", ET_DOMAIN) ?>
						</span>
						<?php } ?>
					</div><!-- END TITLE -->
					<div class="col-md-5 col-sm-6 col-xs-6">
						<ul class="q-f-sort">
		                    ...
		                    <li>
		                        <a class="<?php echo isset($_GET['sort']) && $_GET['sort'] == 'unreacted' ? 'active' : ''; ?>" href="<?php echo add_query_arg(array('sort' => 'unreacted')); ?>">
					...
					<div class="col-md-5 col-sm-6 col-xs-6 categories-wrapper">
                        ...
                        <div class="number-of-stories-wrapper">
        ...
        <div class="row story-filter" id="story_filter">
            <div class="col-md-6 col-xs-6 sort-stories">
                <ul>
                    ...
                    <!--<li>
                        <a class="<?php echo isset($_GET['sort']) && $_GET['sort'] == 'unreacted' ? 'active' : ''; ?>" href="<?php echo add_query_arg(array('sort' => 'unreacted'), is_page_template( 'page-pending.php' ) ? home_url() : $current); ?>"><?php _e("Unreactioned",ET_DOMAIN) ?></a>
                    </li>-->
                    <?php
                    	global $current_user;
                    	if( current_user_can( 'manage_options' ) && et_count_posts("pending") > 0 ){
            ...
            <div class="col-md-6 col-xs-6">
                <div class="number-of-stories-wrapper">
                	<span class="number-of-stories-text"><?php _e("Stories Per Page: ", ET_DOMAIN ); ?></span>
                 	<div class="number-of-stories">
                        <select id="filter-numbers" class="select-grey-bg">
							<?php
								foreach ($posts_per_page as $key => $value) {
...
*
**/
function forgivingheals_comment_post_template($comment, $args, $depth){
...
*
**/
function forgivingheals_tag_template(){
	?>
...
*
**/
function forgivingheals_comment_template(){
...
*
**/
function forgivingheals_reaction_template(){
	// get template-js/item-reaction.php
	get_template_part( 'template-js/item', 'reaction' );
}
/**
*
...
*
**/
function forgivingheals_select_categories($args = array()){
		$args = wp_parse_args( $args, array(
				'hide_empty' => 0,
			));
	?>
	<div class="select-categories">
		<select id="story_category" name="story_category" class="categories-select">
...
*
**/
function forgivingheals_comments_loop($child){
...
/**
*
* TEMPLATE LOOP FOR REACTIONS
* @param array $reactions
* @author ThaiNT
* @since 1.0
*
**/
function forgivingheals_reactions_loop(){
	...
	$reactionsData  = array();
	$commentsData = array();
	$story     = ForgivingHeals_Stories::convert(get_post($story_ID));
	...
	//show pending reaction if current user is admin
	if( is_user_logged_in() && ( forgivingheals_user_can('approve_reaction') || current_user_can( 'manage_options' ) ) ){
...
?>
	...
	<div id="reactions_main_list">
		...
		if($replyQuery->have_posts()){
			while($replyQuery->have_posts()){ $replyQuery->the_post();
				global $post, $forgivingheals_reaction, $forgivingheals_reaction_comments;
				$forgivingheals_reaction          = ForgivingHeals_Reactions::convert($post);
				$reactionsData[]      = $forgivingheals_reaction;
				...
				$commentsData       = array_merge($commentsData, $forgivingheals_reaction_comments);

			?>
			<div class="row story-main-content story-item reaction-item" id="<?php echo $forgivingheals_reaction->ID ?>">
	...
	<script type="text/javascript">
		...
		?>
		var reactionsData  = <?php echo defined('JSON_HEX_QUOT') ? json_encode( $reactionsData, JSON_HEX_QUOT ) : json_encode( $reactionsData ) ?>;
...
*
**/
function forgivingheals_count_post_in_tags($tag){
	$today   = getdate();
	$today_query = new WP_Query( 'post_type=story&forgivingheals_tag='.$tag.'&year=' . $today["year"] . '&monthnum=' . $today["mon"] . '&day=' . $today["mday"] );

	$week  = date('W');
	$year  = date('Y');
	$month = date('m');
	$week_query   = new WP_Query( 'post_type=story&forgivingheals_tag='.$tag.'&year=' . $year . '&w=' . $week );
	$month_query  = new WP_Query( 'post_type=story&forgivingheals_tag='.$tag.'&year=' . $year . '&monthnum=' . $month );
...
*
**/
function forgivingheals_reset_password_modal(){
...
*
**/
function forgivingheals_login_register_modal(){
...
*
**/
function forgivingheals_edit_profile_modal(){
...
*
**/
function forgivingheals_insert_story_modal(){
	get_template_part( 'template/modal', 'story' );
}
/**
 *
 * COUNT REACTIONS OF STORY
 * @param  int $story_id
 * @author ThaiNT
 * @since v1.0
 *
 **/
function et_count_reaction($id){
	global $current_user;
 ...
 * @return string $link
 */
function forgivingheals_search_link($query){
	global $wp_rewrite;
...
function forgivingheals_comment_form ( $post, $type = 'story' ) {
	global $current_user;
	/**
     * check privileges
    */
    ...
    if( !forgivingheals_user_can('add_comment') && isset( $privi->add_comment ) ) {
...
?>
    ...
    <form class="child-reply" method="POST">
        ...
        <div id="editor_wrap_<?php echo $post->ID ?>" class="child-reaction-wrap collapse">
            <div class="wp-editor-container">
                <textarea name="post_content" id="insert_reaction_<?php echo $post->ID ?>"></textarea>
            </div>
...
/**
 * echo tos text in form comment, post reaction
 * @author Dakachi
*/
function forgivingheals_tos ($word) {
	$word == "reaction" ? printf(__('By posting your reaction, you agree to the <a target="_blank" href="%s">privacy policy</a> and <a target="_blank" href="%s">terms of service.</a>', ET_DOMAIN), et_get_page_link('term'), et_get_page_link('term')) : printf(__('By posting your story, you agree to the <a target="_blank" href="%s">privacy policy</a> and <a target="_blank" href="%s">terms of service.</a>', ET_DOMAIN), et_get_page_link('term'), et_get_page_link('term'));
}
...
*
**/
function forgivingheals_report_modal(){
...
*
**/
function forgivingheals_contact_modal(){
	get_template_part('template/modal' , 'contact' );
}
/**
*
* Print Dropdown Story Category
* @param null
* @author thaint
* @since 1.0
*
**/
function forgivingheals_tax_dropdown(){
	?>
    <div class="select-categories-wrapper">
        <div class="select-categories">
            <?php
                wp_dropdown_categories(array(
                    ...
                    'depth'           => 4,
                    'id'              => 'move_to_category',
                    'walker'          => new ForgivingHeals_Walker_TaxonomyDropdown(),
                ));
            ?>
        </div>
    </div>
...
class ForgivingHeals_Walker_TaxonomyDropdown extends Walker_CategoryDropdown{
    ...
    public function start_el( &$output, $category, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $value = get_term_link( $category, 'story_category' );
