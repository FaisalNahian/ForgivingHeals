...
*/
class ForgivingHeals_User_Query extends WP_User_Query {
	/**
...
/**
 * Handle member data in forum forgivingheals
 */
class ForgivingHeals_Member extends ET_User{
	...
	public function __construct(){
		$this->meta_data = array(
			'et_avatar',
			'forgivingheals_following_stories',
			'user_facebook',
			'user_twitter',
			'user_gplus',
			'user_location',
	...
	static public function get_instance(){
		if (self::$instance == null){
			self::$instance = new ForgivingHeals_Member();
		}
	...
	static public function convert($user){
		...
		$result->et_story_count = et_count_user_posts($result->ID, 'story');
		$result->et_reaction_count   = et_count_user_posts($result->ID, 'reaction');
		...
		if ( empty($result->et_story_count) ) $result->et_story_count = 0;
		if ( empty($result->et_reaction_count) ) $result->et_reaction_count = 0;
		// ban expired
		...
		*/
		$result->cap =	forgivingheals_get_user_caps();
	...
	static public function get_current_member(){
		...
		if ( !$user->ID ) return $user;
			...
			return ForgivingHeals_Member::convert($user);
		}
	}
}

function et_get_avatar($id, $size = 64,$params = array('class'=> 'avatar','alt' => '')){
	return ForgivingHeals_Member::get_avatar($id, $size, $params);
}
...
class ET_UserAjax extends AE_Base{
		...
		try{
			...
			if( isset($_POST['do_action'])){
				...
				if( $_POST['do_action'] == "saveProfile" ){
					...
					ForgivingHeals_Member::update(array(
							'ID'            => $_POST['ID'],
					...
					$user = ForgivingHeals_Member::convert(get_userdata( $_POST['ID'] ));
	...
	public function login(){
		$args          = $_POST['content'];
		$user_facetory = ForgivingHeals_Member::get_instance();
		...
		if(!is_wp_error($user)) $userdata  	= ForgivingHeals_Member::convert($user);
	...
	public function register(){
		...
		try {
			...
			$userdata 	= ForgivingHeals_Member::convert($data);
			// generate new nonces
			...
			$response = array(
				...
				'msg' 			=> $msg,
				'data' 			=> $userdata,
				'redirect'		=> apply_filters( 'forgivingheals_filter_redirect_link_after_register', home_url() )
			);
	...
	public function forgot(){
		...
		$user = ForgivingHeals_Member::convert($user_data);
	...
	public function inbox(){
		...
		try {
			...
			$subject     = apply_filters('forgivingheals_inbox_subject',__('New Private Message From ', ET_DOMAIN).get_bloginfo('blogname' ));
	...
	public function ban_user(){
		try {
			...
			$user      = ForgivingHeals_Member::get_instance();
...
function et_make_member_data($user){
	...
	$info = (array)$user->data + array(
		'id' 				=> $user->ID,
		'story_count' 	=> get_user_meta($user->ID, 'et_story_count',true) ? get_user_meta($user->ID, 'et_story_count',true) : 0,
		'reaction_count' 		=> get_user_meta($user->ID, 'et_reaction_count', true) ? get_user_meta($user->ID, 'et_reaction_count',true) : 0,
	...
	$member_object = ForgivingHeals_Member::get_instance();
 ...
 */
function et_count_user_posts($user_id,$post_type = "story"){
	global $wpdb;
...
function et_unban_expired_users(){
	...
	$member = ForgivingHeals_Member::get_instance();
	...
	foreach ($users as $user) {
		ForgivingHeals_Member::unban( $user->ID );
	}
}
