...
abstract class ET_SocialAuth extends AE_Base{
	...
	protected function _create_user($params){
		// insert user
		$result = ForgivingHeals_Member::insert( $params );
		...
		try {
			...
			if ( $user == false ){
				...
				if ( isset($auth_info['user_login']) ){
					...
					if ( $user == false ){
						$result = ForgivingHeals_Member::insert($auth_info);
		...
		// twitter auth callback
 		...
 		// create user
			...
			if ( !empty($_POST['user_email']) ){
				...
				$result = ForgivingHeals_Member::insert( $new_account );
				if ( !is_wp_error( $result ) ){
					// send email here
					do_action('et_after_register', $result);
					// login
	...
	public function auth_facebook(){
		try {
			...
			if ( isset($data['id']) && $user = $this->get_user($data['id']) ){
				...
				$userdata 	= ForgivingHeals_Member::convert( $user );
				$nonce 		= array(
