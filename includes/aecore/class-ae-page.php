	...
	public function print_scripts () {
		...
		wp_localize_script( 'appengine', 'ae_globals', 
						array(
							...
							'pending_reactions' => ae_get_option("pending_reactions", 0),
	...
	?>
		...
		<div class="wrap">
			<div class="et-body">
				<div class="et-header">
					<div class="logo">
						<a href="http://www.maxweb.us/"> Powered by <img src="<?php echo ae_get_url(); ?>/assets/img/forgivingheals-logo.png" /> </a>
					</div>
	...
	public function render_menu () {
		...
		if(!empty($this->pages)) {
			...
			foreach ($this->pages as $key => $page) {
				...
				echo '<li>
						<a class="forgivingheals-menu '. $active .'" href="?page='. $args['slug'] .'">
							<div class="forgivingheals-menu-icon icon-'. $args['icon'] .'"></div><div class="">'. $args['menu_title'] .'</div>
						</a>
					</li>';
				$active	=	'';
			}
...
class AE_Menu extends AE_Page {
	...
	function __construct ( $pages ) {
		...
		*/
		...
		$this->args = array(
			'page_title' 	=> __('ForgivingHeals Settings', ET_DOMAIN),
			'menu_title' 	=> __('ForgivingHeals Settings', ET_DOMAIN),
			'cap' 			=> 'administrator',
			'slug' 			=> 'et-overview',
			'icon_url' 		=> '',
			'pos' 			=> 3
