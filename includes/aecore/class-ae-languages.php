...
class AE_Language extends AE_Base {
	...
	*/
	function __construct () {
		...
		$this->_file_name 	= 'forgivingheals';
		// $this->_pot			= new MO();
	...
	*/
	...
	/**
	 *	create a directory et-content in wp-content to store MaxWeb content
	...
	*/
	...
	function generate_pot () {
		...
		// // set file header
		// $pot->set_header( 'Project-Id-Version', 'Job ForgivingHeals v'.CE_VERSION);
		...
		// $pot->set_header( 'Last-Translator', 'MaxWeb <contact@maxweb.us>' );
		// $pot->set_header( 'Language-Team', 'MaxWeb <contact@maxweb.us>' );
	...
	function generate_mo ( $file_name ) {
		...
		$mo->set_header( 'Last-Translator', 'MaxWeb <contact@maxweb.us>' );
		$mo->set_header( 'Language-Team', 'MaxWeb <contact@maxweb.us>' );
	...
	/**
	 * generate translate string from forgivingheals.po
	 * @since 1.0
	*/
	function get_translate_string () {
		$this->_pot		=	new PO();
		$this->generate_pot();
		$this->_pot->import_from_file(DEFAULT_LANGUAGE_PATH.'/forgivingheals.po',true );
	...
	*/
	function save_language () {
		...
		foreach ($language_files as $type => $mo) {
			...
			$mo->set_header( 'Language-Team', 'maxweb.us <faisalmaxweb.us>' );
			...
			if($type == 'mo') { // mo file
				...
				if(file_exists( THEME_LANGUAGE_PATH.'/'.$selected_lang.'.po' )) {
					...
					$mo->import_from_file( DEFAULT_LANGUAGE_PATH.'/forgivingheals.po' );	
				}				
			}
	...
	function load_translation_form () {
		...
		$pot		=	new PO();
		/*et_generate_pot();
		$pot->import_from_file(DEFAULT_LANGUAGE_PATH.'/forgivingheals.po',true );
		*/
		$translated	=	array ();	
		$mo			=	new MO();
