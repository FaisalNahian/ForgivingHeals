...
class ForgivingHeals extends AE_Base{
	...
	function __construct(){
		...
		$this->add_action('forgivingheals_accept_reaction'				, 'forgivingheals_after_accept_reaction', 10, 2);
		...
		$this->add_filter('wp_title'						, 'forgivingheals_wp_title', 10, 2 );
		$this->add_filter('excerpt_length'					, 'forgivingheals_excerpt_length' );
		$this->add_filter('excerpt_more'					, 'forgivingheals_excerpt_more' );
		...
		$this->add_action( 'et_insert_story'			, 'store_new_story_to_DB');

		}

		$this->add_action( 'et_insert_story'			, 'alert_pending_story_to_admin' );

		$this->add_action( 'et_insert_story'			, 'save_following_stories');
		$this->add_action( 'et_insert_reaction'			, 'save_following_stories' );
		//$this->add_action( 'ae_admin_user_action' 		, 'add_user_actions_backend');
		$this->add_action( 'forgivingheals_send_following_mail' 	, 'mail_to_following_users' );
		$this->add_action( 'add_meta_boxes'				, 'add_post_meta_box' );
		$this->add_action( 'et_after_reported'			, 'et_reported_email', 10, 2 );
		$this->add_filter( 'wp_link_query_args'			, 'forgivingheals_tinymce_filter_link_query' );

		//add return field for user
		$this->add_filter( 'ae_convert_user'			, 'ae_convert_user' );

		if(ae_get_option('forgivingheals_send_following_mail' ) && !ae_get_option("pending_reactions"))
			$this->add_action( 'et_insert_reaction'  ,'forgivingheals_stories_new_reaction' );

		//short codes
		new ForgivingHeals_Shortcodes();
		...
		$this->add_filter( 'forgivingheals_wp_title', 'ae_filter_badword' );
		/**
		 * bind ajax to get tag json for autocomplete tag in modal add/edit story
		*/
		$this->add_ajax('forgivingheals_get_tags' , 'forgivingheals_get_tags');
	...
	/*
	* Send email to reaction author when the reaction is the best
	*
	*/
	function forgivingheals_after_accept_reaction($reactionID, $action){
		//get post data
		$reaction       = get_post( $reactionID );
		$story     = get_post( $reaction->post_parent );
		$author       = get_user_by( 'id', $reaction->post_author );

		$author_email = $author->user_email;

		$message = ae_get_option('accept_reaction_mail_template');
		$message = stripslashes($message);
		$message = str_ireplace('[action]', $action == "accept-reaction" ? __("marked", ET_DOMAIN) : __("unmarked", ET_DOMAIN), $message);
		$message = str_ireplace('[display_name]', $author->display_name, $message);
		$message = str_ireplace('[story_link]', get_permalink( $story->ID ), $message);
		$message = str_ireplace('[blogname]', get_option('blogname'), $message);

		$subject =	sprintf(__("[%s] Your reaction has been marked as the best.",ET_DOMAIN),get_option('blogname'));
	...
	function ae_convert_user($user){
		$user_instance           = ForgivingHeals_Member::get_instance();
		...
		$user->forgivingheals_point          = get_user_meta($user->ID, 'forgivingheals_point', true) ? get_user_meta($user->ID, 'forgivingheals_point', true) : 0;
		$user->et_story_count = et_count_user_posts($user->ID, 'story');
		$user->et_reaction_count   = et_count_user_posts($user->ID, 'reaction');
		return $user;
	}
	function forgivingheals_tinymce_filter_link_query($query){
		$query['post_type']   =	'story';
		$query['post_status'] =	array('publish','closed');
		return $query;
	}
	/**
	 *  Send email to admin after new pending story created
	 */
	function alert_pending_story_to_admin($id){
		if(ae_get_option( 'pending_stories' ) && get_post_status( $id ) == "pending"){
			$admin_email = apply_filters( 'email_alert_pending_story', get_option('admin_email') );

			$message =	ae_get_option('pending_story_mail_template');
			$message = 	stripslashes($message);
			$message =	str_ireplace('[story_title]', get_the_title( $id ), $message);
			$message =	str_ireplace('[pending_story_link]', et_get_page_link("pending"), $message);
			$message =	str_ireplace('[blogname]', get_option('blogname'), $message);

			$subject =	sprintf(__("[%s] New pending story has been created.",ET_DOMAIN),get_option('blogname'));
	...
	/**
	 * Update option story has a new reaction
	 */
	static public function forgivingheals_stories_new_reaction($id){
		$reaction    = get_post( $id );
		$id        = $reaction->post_parent;
		$stories = (array)get_option( 'forgivingheals_stories_new_reaction' );
		if(is_array($stories))
			array_push( $stories , $id);
		update_option( 'forgivingheals_stories_new_reaction' , array_filter(array_unique( $stories )) );
	}
	/**
	 * Email to following user when thread has new reply
	 */
 	public function mail_to_following_users(){
		$stories = get_option( 'forgivingheals_stories_new_reaction' );
		global $current_user;

		if(!empty($stories)){
			foreach ($stories as $id) {

				$story_title = get_the_title($id);
				...
				foreach ($users_follow as $userid) {
					...
					$headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
					$headers .= "From: ".get_option('blogname')." < ".get_option('admin_email') ."> \r\n";
					$subject = sprintf(__("[New Reaction] Your story %s has a new reply.",ET_DOMAIN), $story_title);

					$message = ae_get_option('new_reaction_mail_template');
					$message = stripslashes($message);
					/* ============ filter placeholder ============ */
					$message =	str_ireplace('[display_name]', $user->display_name, $message);
					$message =	str_ireplace('[story_title]', $story_title, $message);
					$message =	str_ireplace('[story_link]', get_permalink($id), $message);
					$message =	str_ireplace('[blogname]', get_option('blogname'), $message);
					/* ============ filter placeholder ============ */
			...
			update_option( 'forgivingheals_stories_new_reaction' , array() );
		}
	}
	/**
	 * Save thread id to following to usermeta
	 */
	public function save_following_stories($id){
		global $user_ID;

		if(get_post_type($id) != "reaction" && get_post_type($id) != "story") {
			return;
		}

		if(get_post_type($id) == "reaction"){
			$reaction = get_post( $id );
			$id     = $reaction->post_parent;
			//update last author to story
			update_post_meta( $id, 'et_last_author', $reaction->post_author );
		}
		...
		ForgivingHeals_Stories::update_field($id, 'et_users_follow', $users_follow);
	}
	/* ==================== LIVE NOTIFICATION ==================== */
	public function send_data_to_heartbeat($response, $data){
		...
		$sql = $wpdb->prepare(
			...
			'_transient_forgivingheals_notify_%'
		);
		...
		if(!empty($notifications)){
			foreach ( $notifications as $db_notification ) {
				...
				if(ae_get_option( 'pending_stories' )){
	...
	public function store_new_story_to_DB($post_id){
		...
		//if( get_option( 'pending_stories' ) && !current_user_can( 'administrator' ))
			//return $post_id;
	...
	public function forgivingheals_excerpt_length(){
		return 20;
	}
	public function forgivingheals_excerpt_more( $more ) {
		return ' ...';
	}
	public function forgivingheals_wp_title( $title, $sep ) {
		global $paged, $page;
		...
		return apply_filters( 'forgivingheals_wp_title', $title);
	}
	...
	public function set_default_theme(){
		...
		if( is_admin() && isset($_GET['activated'] ) && $pagenow == 'themes.php' ){
			if( !get_option( 'forgivingheals_first_time_active' ) ){
				//add default page:
				...
				if ( empty($front_id) ){
					$front = wp_insert_post(array(
						'post_status' => "publish",
						'post_type'   => 'page',
						'post_title'  => 'Stories Listing'
					));
					update_option( 'page_on_front' , $front );
					update_post_meta( $front, '_wp_page_template', 'page-stories.php' );
				}
				...
				update_option( 'forgivingheals_first_time_active', 1 );
			}
		}
	}

	public function et_widgets_init(){
		register_widget('ForgivingHeals_Hot_Stories_Widget');
		register_widget('ForgivingHeals_Statistic_Widget');
		register_widget('ForgivingHeals_Tags_Widget');
		register_widget('ForgivingHeals_Recent_Activity');
		register_widget('ForgivingHeals_Top_Users_Widget');
		register_widget('ForgivingHeals_Related_Stories_Widget');
	}
	...
	public function default_user_meta( $user_id, $role = false) {
		...
		update_user_meta( $user_id, 'forgivingheals_point', apply_filters('forgivingheals_default_points_after_register', 1) );
		update_user_meta( $user_id, 'et_story_count', 0 );
		update_user_meta( $user_id, 'et_reaction_count', 0 );
	...
	public function init_theme(){
		global $wp_rewrite;
		// post type
		ForgivingHeals_Stories::init();
		ForgivingHeals_Reactions::init();
		ForgivingHeals_Member::init();
		...
		/**
		 * new class ForgivingHeals_PackAction to control all action do with user badge
		*/
		$forgivingheals_pack = new ForgivingHeals_PackAction();
		...
		register_sidebar( array(
			...
			'id' 			=> 'forgivingheals-header-sidebar',
		...
		register_sidebar( array(
			...
			'id' 			=> 'forgivingheals-blog-left-sidebar',
			'description' 	=> __("Display widgets in blog's left sidebar", ET_DOMAIN)
		) );
		register_sidebar( array(
			'name' 			=> __('Blog\'s Right Sidebar', ET_DOMAIN),
			'id' 			=> 'forgivingheals-blog-right-sidebar',
			'description' 	=> __("Display widgets in blog's right sidebar", ET_DOMAIN)
		) );

		//single story sidebars
		register_sidebar( array(
			'name' 			=> __('Single Story Sidebar', ET_DOMAIN),
			'id' 			=> 'forgivingheals-story-right-sidebar',
			'description' 	=> __("Display widgets in single story sidebar", ET_DOMAIN)
		) );

		register_sidebar( array(
			'name' 			=> __('Top Stories Listing Ads Banner Sidebar', ET_DOMAIN),
			'id' 			=> 'forgivingheals-top-stories-banner-sidebar',
			'description' 	=> __("Display ad banners widgets in top stories listing sidebar", ET_DOMAIN)
		) );

		register_sidebar( array(
			'name' 			=> __('Bottom Ads Banner Sidebar', ET_DOMAIN),
			'id' 			=> 'forgivingheals-btm-stories-banner-sidebar',
			'description' 	=> __("Display ad banners widgets in bottom of website", ET_DOMAIN)
		) );

		register_sidebar( array(
			'name' 			=> __('Content Story Ad Banner Sidebar', ET_DOMAIN),
			'id' 			=> 'forgivingheals-content-story-banner-sidebar',
			'description' 	=> __("Display ad banners widgets in bottom stories listing sidebar", ET_DOMAIN)
		) );

		register_sidebar( array(
			'name' 			=> __('Below Reactions Listing Ad Banner Sidebar', ET_DOMAIN),
			'id' 			=> 'forgivingheals-btm-single-story-banner-sidebar',
			'description' 	=> __("Display ad banners widgets in bottom stories listing sidebar", ET_DOMAIN)
		) );
	...
	public function on_add_scripts(){
		...
		$isEditable = current_user_can( 'manage_stories' );
		$variables = array(
			...
			'pending_stories' => ae_get_option('pending_stories') ? 1 : 0,
			'pending_reactions'   => ae_get_option("pending_reactions") ? 1 : 0,
	...
	public function forgivingheals_get_tags() {
		$terms	=	get_terms('forgivingheals_tag', array('hide_empty' => 0, 'fields' => 'names' )) ;
		wp_send_json($terms);
	}
	/**
...
class ForgivingHeals_Shortcodes{
...
function editor_settings($args = array()){
	$buttons = apply_filters( 'forgivingheals_editor_buttons', 'bold,|,italic,|,underline,|,link,unlink,|,bullist,numlist,forgivinghealsimage,forgivinghealscode' );
	return array(
	'quicktags' 	=> false,
	'media_buttons' => false,
	'tabindex' 		=> 5,
	...
	'tinymce' 		=> array(
		...
		'setup'                 => 'function(ed) {
			...
			ed.on("focus", function(e) {
				if(currentUser.ID == 0)
					ForgivingHeals.App.openAuthModal(e);
			});
 ...
 * @uses Walker
 */
class ForgivingHeals_Custom_Walker_Nav_Menu extends Walker_Nav_Menu {
	/**
...
function et_count_posts($status = 'publish', $type = 'story'){
...
*
**/
function forgivingheals_static_texts(){
	return 	array(
		'form_auth'	=> array(
		...
		'texts' => array(
			...
			'best_ans_txt'    => __("Best reaction", ET_DOMAIN),
			'forgotpass'      => __("Forgot Password", ET_DOMAIN),
			'close_tab'       => __("You have made some changes which you might want to save.", ET_DOMAIN),
			'confirm_account' => __("You must activate your account first to create stories / reactions!.", ET_DOMAIN),
