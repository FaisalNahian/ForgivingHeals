(function (Views, Models, $, Backbone) {
	Views.PostListItem = Backbone.View.extend({
		tagName: 'section',
		className: 'list-reactions-wrapper reaction-item',
		model : [],
		events: {
			'click a.action'  	 		 		 : 'doAction',
			'click a.mb-show-comments'	 		 : 'showCommentsList',
			'click a.add-cmt-in-cmt'	 		 : 'showCommentForm',
			'click a.close-form-post-reactions'	 : 'hideCommentForm',
			'submit form.create-comment'	 	 : 'insertComment',
		},
		initialize: function(){
			if($('#mobile_reaction_item').length > 0){
				this.template =  _.template($('#mobile_reaction_item').html());
			}
			this.blockUi	=	new AE.Views.BlockUi();

			// if( currentUser ) {
			// 	this.currentUser = ForgivingHeals.MobileApp.currentUser ;
			// }
		},
		...
		doAction: function(event){
			...
			if( typeof userCaps[action] === 'undefined' &&
				...
				!( action == 'accept-reaction' || action == 'un-accept-reaction' || action == 'approve') ) {
				AE.pubsub.trigger('ae:notification', {
					msg: forgivingheals_front.texts.enought_points,
					notice_type: 'error',
				});
				return false;
			}
			...
			if(action == "vote_up" || action == "vote_down"){
			  ...
			  /* ========== ON MARK ACCEPT ========== */
				...
				if(target.hasClass('best-reactions')) {
					target.removeClass('best-reactions')
						.addClass('pending-reactions')
						.text(forgivingheals_front.texts.accept_txt)
						.attr('data-name', 'accept-reaction');
				}else{
					$('a.reaction-active-label').not('.has-best-reaction').removeClass('best-reactions')
											.addClass('pending-reactions')
											.text(forgivingheals_front.texts.accept_txt)
											.attr('data-name', 'accept-reaction');
					target.addClass('best-reactions')
						.removeClass('pending-reactions')
						.html('<i class="fa fa-check"></i>'+forgivingheals_front.texts.best_ans_txt)
						.attr('data-name', 'un-accept-reaction');
				}
			  /* ========== ON EDIT POST ========== */
	...
	Views.MobileFront = Backbone.View.extend({
		...
		initialize: function(){
			 ...
			 * tags list container
			*/
			this.tag_list = this.$('ul.post-story-tags');
		...
		saveStory: function(event){
			...
			*/
			...
			var form = $(event.currentTarget),
			$button  = form.find("button.submit-post-story"),
			textarea = form.find('textarea'),
			data     = form.serializeObject(),
			view     = this;
		...
		*/
		addTag: function(tag){
			...
			if ( duplicates.length == 0 && tag != '' && count.length < 5 ){
				...
				$('input#story_tags').val('').css('border', 'none');
			}
		},
